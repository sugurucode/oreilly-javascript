- 標準入力 (Standard Input / stdin)
  プログラムがデータを受け取るための「標準的な入り口」。デフォルトではキーボードからの入力に設定されている。

- 標準出力 (Standard Output / stdout)
  プログラムが正常な実行結果を出力するための「標準的な出口」。デフォルトではターミナル画面に設定されている。

- 標準エラー出力 (Standard Error / stderr)
  プログラムがエラーメッセージや警告を出力するための「エラー専用の出口」。標準出力と同じくデフォルトはターミナル画面だが、標準出力とは別の経路になっているため、正常な結果とエラーを分けて扱うことができる。

- リダイレクト (Redirect)
  標準入出力の「向き先」を変更する機能です。（例：標準出力をファイルにリダイレクト）

- パイプ (Pipe : |)
  あるプログラムの「標準出力」を、別のプログラムの「標準入力」へ直接つなぐ。バケツリレーのようにデータを渡していくことができる。
  ex04の例↓
