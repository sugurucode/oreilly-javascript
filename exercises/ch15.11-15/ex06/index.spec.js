...
test.beforeEach(async ({ page }) => {
  await page.goto('http://localhost:3000/ch15.11-15/ex05/index.html');
});
...
// このテストはsessionStorage版では動作しないためスキップ
test.skip('別タブと同期される', async ({ context, page }) => {
  // page は「タブA」

  // 「タブB」を新しく作る
  // contextとはブラウザのウィンドウに相当し、その中に複数のタブ(page)を作れる
  const page2 = await context.newPage();
  await page2.goto('http://localhost:3000/ch15.11-15/ex05/index.html');
