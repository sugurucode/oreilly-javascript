...
test.describe('div & input Site', () => {
  ...
  test('When it focus the input, the color of the div is silver', async ({ page }) => {
    await gotoTestTarget(page);
    await expect(getDiv(page)).toHaveCSS('background-color', 'rgb(255, 255, 255)');
    await getDiv(page).click();
    await expect(getDiv(page)).toHaveCSS('background-color', 'rgb(192, 192, 192)');
